...
Prepare:
 python tests/audit_harmonic_reference_validation_v1.py prepare \
   --detective-v2 tests/harmonic_detective_v2.zip --wav-root tests \
   --out tests/harmonic_reference_v1
...
Evaluate:
 python tests/audit_harmonic_reference_validation_v1.py evaluate \
   --out tests/harmonic_reference_v1
...
FIELDS=['item_id','song','time_s','excerpt_file','reference_status','reference_midi','reference_confidence','condition','transition','annotation_notes']
...
def prepare(a):
    if a.seed<0 or a.cluster_seconds<=0 or a.clip_seconds<=0:raise ValueError('Invalid settings')
    ...
    for i,r in enumerate(selected,1):
        ...
        excerpt=f'clips/{id_}.wav';sf.write(a.out/excerpt,data[lo:hi],sr,subtype='PCM_16')
        ...
        annotations.append({'item_id':id_,'song':song,'time_s':f'{t:.6f}','excerpt_file':excerpt,
    ...
    manifest={'protocol':'harmonic_reference_validation_v1','source_v2':str(a.detective_v2),
...
def main():
    ...
    a.add_argument('--wav-root',type=Path,default=Path('tests'));a.add_argument('--out',type=Path,default=Path('tests/harmonic_reference_v1'))
    ...
    b=sp.add_parser('evaluate');b.add_argument('--out',type=Path,default=Path('tests/harmonic_reference_v1'))
    args=p.parse_args()
