#!/usr/bin/env python3
"""
V22 V10: complete tournament with a conservative close-frequency referee.
...
def close_frequency_referee(waveform, t, low, high, cfg, interruptions):
    """Independent long-window lag-peak check; NEVER uses V21 exclusive energy.

    Only claim a winner when three longer waveform windows independently yield a
    stable period matching the same candidate. Otherwise abstain explicitly.
    Longer windows can span note boundaries, so inconsistent windows abstain.
    """
    import numpy as np
    y, sr = waveform
    width = cfg.close_window_ms / 1000.0
    estimates = []
    report = {"method": "long_window_fractional_lag_peak", "window_ms": cfg.close_window_ms,
              "estimates_hz": [], "reason": None, "winner_hz": None}
    for offset in (-cfg.close_shift_ms/1000.0, 0.0, cfg.close_shift_ms/1000.0):
        center = t + offset
        left, right = center - width/2, center + width/2
        if left < 0 or right >= len(y)/sr:
            report["reason"] = "window_outside_wav"
            return report
        # Never cross an explicitly flagged interruption.
        if any(left < cut < right for cut in interruptions):
            report["reason"] = "window_crosses_interruption"
            return report
        x = np.asarray(y[round(left*sr):round(right*sr)], dtype=np.float64)
        x -= x.mean()
        if len(x) < 64 or np.sqrt(np.mean(x*x)) < cfg.close_min_rms:
            report["reason"] = "insufficient_local_energy"
            return report
        # Peak of normalized autocorrelation, restricted to a tight union of
        # candidate periods. Fractional-lag interpolation avoids integer-lag bias.
        lag_min = max(2, int(np.floor(sr/(high*1.04))))
        lag_max = min(len(x)//3, int(np.ceil(sr/(low*0.96))))
        if lag_max <= lag_min:
        ...
        for lag in range(lag_min, lag_max+1):
            ...
            values.append(float(np.dot(a,b)/denom) if denom > 1e-18 else -1.0)
        k = int(np.argmax(values))
        lag = lag_min + k
        if k == 0 or k == len(values)-1:
            report["reason"] = "peak_at_search_boundary"
            return report
        a,b,c = values[k-1:k+2]
        curvature = a-2*b+c
        delta = 0.5*(a-c)/curvature if curvature < -1e-12 else 0.0
        delta = float(np.clip(delta, -.5, .5))
        freq = sr/(lag+delta)
        if values[k] < cfg.close_min_peak_acf:
            report["reason"] = "weak_long_window_periodicity"
            return report
        estimates.append(freq)
    report["estimates_hz"] = [round(v, 6) for v in estimates]
    spread = abs(cents(max(estimates), min(estimates)))
    report["spread_cents"] = spread
    if spread > cfg.close_max_spread_cents:
        report["reason"] = "nonstationary_pitch_or_note_transition"
        return report
    est = median(estimates)
    dlo, dhi = abs(cents(est,low)), abs(cents(est,high))
    report["median_estimate_hz"] = est
    report["distance_low_cents"] = dlo
    report["distance_high_cents"] = dhi
    if min(dlo,dhi) > cfg.close_match_cents or abs(dlo-dhi) < cfg.close_separation_cents:
        report["reason"] = "no_clear_candidate_match"
        return report
    report["winner_hz"] = low if dlo < dhi else high
    report["reason"] = "stable_direct_period_match"
    return report
...
def complete_head_to_head(case, t, records, waveform, cfg, refs, rinfo, interruptions, v21_measure):
    ...
    for i in range(len(centers)):
        for j in range(i + 1, len(centers)):
            ...
            result["case"] = case
            close = abs(high - low) < (waveform[1] / round(0.064 * waveform[1]))
            result["close_pair_below_64ms_fft_bin"] = close
            result["comparison_origin"] = "new_direct_wav_v21_measurement"
            result["synthetic_comparison"] = False
            result["close_referee_json"] = ""
            if close and result["low_range_admitted"] and result["high_range_admitted"]:
                # V21's low-exclusive harmonic bins are not identifiable at
                # these spacings. Do not let that score decide a close match.
                ref = close_frequency_referee(waveform, t, low, high, cfg, interruptions)
                result["close_referee_json"] = json.dumps(ref, sort_keys=True)
                result["comparison_origin"] = "close_direct_long_window_referee"
                if not result["low_admissible"] or not result["high_admissible"]:
                    # Do not override the existing acoustic admission gate.
                    result["winner_hz"] = None
                    result["outcome"] = "abstain_close_candidate_not_admissible"
                elif ref["winner_hz"] is None:
                    result["winner_hz"] = None
                    result["outcome"] = "abstain_close_frequency_unresolved"
                else:
                    # This is independent periodicity corroboration, not a
                    # reweighted V21 exclusive-energy or contextual victory.
                    result["winner_hz"] = ref["winner_hz"]
                    result["outcome"] = "provisional_low_close_referee" if ref["winner_hz"] == low else "provisional_high_close_referee"
                # Legacy pair scores remain visible for audit only and are
                # explicitly NOT used to decide this close-frequency edge.
                result["legacy_scores_not_used_for_close_decision"] = True
...
def main():
    ...
    ap.add_argument("--close-window-ms", type=float, default=192.0)
    ap.add_argument("--close-shift-ms", type=float, default=12.0)
    ap.add_argument("--close-min-rms", type=float, default=1e-5)
    ap.add_argument("--close-min-peak-acf", type=float, default=0.72)
    ap.add_argument("--close-max-spread-cents", type=float, default=25.0)
    ap.add_argument("--close-match-cents", type=float, default=35.0)
    ap.add_argument("--close-separation-cents", type=float, default=30.0)
    args = ap.parse_args()
    ...
    manifest = {
        "schema": "v22_v10_close_frequency_referee_shadow",
        "close_referee": "three_192ms_fractional_lag_peaks_consensus_or_abstain_no_exclusive_energy",
    ...
    for case in cases:
        ...
        manifest["cases"][case] = {
            ...
            "close_referee_outcomes": dict(__import__("collections").Counter(r["outcome"] for r in extra_rows if r["close_pair_below_64ms_fft_bin"])),
