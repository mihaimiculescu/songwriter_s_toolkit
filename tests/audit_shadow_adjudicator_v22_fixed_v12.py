#!/usr/bin/env python3
"""
V22 V12: V11 evidence, note-group qualification and representative-only tournament.
...
def grouped_note_tournament(records, tolerance_cents, note_tolerance=49.0):
    """V12: acoustic admission -> note groups -> ACF representatives -> V11 inter-note graph.

    Never inherit an eliminated contestant's pair victories. Out-of-tolerance
    candidates stay separate as non-shortcut contestants, rather than disappearing.
    All verdicts are provisional and this is shadow-only.
    """
    import statistics
    frequencies = [hz for r in records for hz, accepted in
                   ((r['low_hz'], r['low_range_admitted']),
                    (r['high_hz'], r['high_range_admitted'])) if accepted]
    details = {'note_groups_json':'[]', 'note_group_count':0,
               'note_group_ambiguities':0, 'note_representative_comparisons':0,
               'note_group_candidate_count':0, 'note_group_eliminated_count':0,
               'note_group_policy':'nearest_note_strict_49c_group_then_median_acf_then_direct_inter_note_graph'}
    if not frequencies:
        status, selected, base = tournament_adjudicate(records,tolerance_cents)
        return status, selected, {**base,**details}
    clusters=cluster_frequency(frequencies,tolerance_cents)
    centers=[median(cluster) for cluster in clusters]
    def node(hz):
        matches=[i for i,c in enumerate(centers)
                 if (d:=cents(hz,c)) is not None and abs(d)<=tolerance_cents]
        return matches[0] if len(matches)==1 else None
    # Collect one median ACF per underlying recorded candidate measurement,
    # then take the median of those medians for its consistent per-candidate score.
    measurements={i:[] for i in range(len(centers))}
    admitted={i:False for i in range(len(centers))}
    for r in records:
        for side in ('low','high'):
            if not r[side+'_range_admitted']: continue
            i=node(r[side+'_hz'])
            if i is None:
                return 'abstain_note_group_unmapped',None,{**details,'note_group_mapping_error':True}
            if r[side+'_admissible']:
                admitted[i]=True
                value=r.get(side+'_acf_median')
                if value is not None and math.isfinite(value): measurements[i].append(value)
    groups=defaultdict(list)
    assignments=[]
    for i,hz in enumerate(centers):
        note=nearest_midi(hz)
        target=440*2**((note-69)/12)
        offset=cents(hz,target)
        if offset is not None and abs(offset)<note_tolerance:
            key=('note',note)
        else:
            key=('outside',i) # No arbitrary assignment and no disappearance.
        groups[key].append(i)
        assignments.append({'node':i,'hz':hz,'nearest_midi':note,'cents':offset,
                            'within_49c':key[0]=='note','acoustically_admissible':admitted[i],
                            'acf_median':median(measurements[i]),
                            'acf_record_count':len(measurements[i])})
    reps=[]; descriptions=[]; ambiguous=0
    for key,members in sorted(groups.items(),key=lambda x:(str(x[0][0]),x[0][1])):
        eligible=[i for i in members if admitted[i] and measurements[i]]
        scores={i:median(measurements[i]) for i in eligible}
        if not eligible:
            descriptions.append({'key':list(key),'members':[assignments[i] for i in members],
                                 'representative_hz':None,'reason':'no_acoustically_supported_candidate'})
            continue
        maximum=max(scores.values())
        winners=[i for i in eligible if scores[i]==maximum]
        if len(winners)!=1:
            ambiguous+=1
            descriptions.append({'key':list(key),'members':[assignments[i] for i in members],
                                 'representative_hz':None,'reason':'top_median_acf_tie'})
            continue
        champion=winners[0]
        reps.append(champion)
        descriptions.append({'key':list(key),'members':[assignments[i] for i in members],
                             'representative_hz':centers[champion],
                             'reason':'singleton_acoustic_admission' if len(members)==1 else 'highest_median_acf'})
    details.update(note_groups_json=json.dumps(descriptions,separators=(',',':')),
                   note_group_count=len(groups),note_group_candidate_count=len(centers),
                   note_group_eliminated_count=len(centers)-len(reps),
                   note_group_ambiguities=ambiguous)
    if ambiguous:
        return 'abstain_note_group_acf_tie',None,details
    if not reps:
        return 'abstain_note_groups_no_acoustically_supported_representative',None,details
    if len(reps)==1 and len(groups)==1:
        details['tournament_champion_frequency_hz']=centers[reps[0]]
        return 'provisional_tournament_champion',centers[reps[0]],details
    if len(reps)!=len(groups):
        return 'abstain_note_group_without_representative',None,details
    # Rebuild cross-note competitions ONLY from directly recorded comparisons
    # involving BOTH actual representatives. Never transfer an eliminated node's win.
    rep_set=set(reps)
    cross=[]
    seen=set()
    for r in records:
        if not (r['low_range_admitted'] and r['high_range_admitted']):continue
        lo,hi=node(r['low_hz']),node(r['high_hz'])
        if lo is None or hi is None:
            return 'abstain_note_group_unmapped',None,details
        if lo==hi or lo not in rep_set or hi not in rep_set:continue
        pair=tuple(sorted((lo,hi)))
        seen.add(pair)
        winner=node(r['winner_hz']) if r['winner_hz'] is not None else None
        cross.append({'low_hz':centers[lo], 'high_hz':centers[hi],
                      'low_range_admitted':True,'high_range_admitted':True,
                      'low_admissible':r['low_admissible'], 'high_admissible':r['high_admissible'],
                      'winner_hz':centers[winner] if winner in (lo,hi) else None})
    details['note_representative_comparisons']=len(cross)
    details['note_representative_missing_pairs']=len(reps)*(len(reps)-1)//2-len(seen)
    if details['note_representative_missing_pairs']:
        return 'abstain_note_representative_unmeasured',None,details
    # Use the untouched V11 directed-graph rules on representative-only edges;
    # near-zero clustering keeps distinct representatives distinct.
    status,selected,old=tournament_adjudicate(cross,1e-7)
    details.update(old)
    return status,selected,details
...
def main():
    ...
    manifest = {
        "schema": "v22_v12_note_group_representative_shadow",
        "close_referee_policy": "V11_pair_measurements_preserved; V12_observation_groups_nearest_midi_strict_49c_selects_representative_by_median_acf",
        "missing_head_to_head_policy": "measure_all_missing_admitted_cluster_pairs_from_original_wav_with_v21_probes; require direct representative pairs",
    ...
    for case in cases:
        ...
        for t, records in sorted(observations.items()):
            status, selected, tournament_details = grouped_note_tournament(
