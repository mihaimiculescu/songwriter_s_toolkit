#!/usr/bin/env python3
"""
V22 V6: V5 spectral baseline plus soft half-cosine range and bidirectional context.
...
def spectral_components(row, cfg):
    ...
    return {
        ...
        "spectral_scoring_model": "symmetric_no_high_intercept_v5",
    }
...
def tournament_adjudicate(records, tolerance_cents):
    ...
    """
    frequencies = [hz for r in records for hz in (r["low_hz"], r["high_hz"])]
    ...
    if unassigned or not edges:
        return ("abstain_tournament_unmapped" if unassigned else "abstain_no_decisive_pair", None, detail)
...
def main():
    ...
    manifest = {
        "schema": "v22_v6_range_bidirectional_interruption_shadow",
        "spectral_model": "symmetric_no_high_intercept_v5",
