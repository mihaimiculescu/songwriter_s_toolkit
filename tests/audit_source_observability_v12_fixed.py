#!/usr/bin/env python3
"""V12 corrected: pre-decision interval challenge and strict adjacent references.
...
def acoustic_assessment(row,items,attribution):
    ...
    # PRE-DECISION acoustic eligibility: energy alone cannot disqualify a pitch.
    # The preceding candidate must exist independently of the curve.
    provisional=(len(eligible)==1 and len(stable)==1 and
                 'temporal_support_asymmetric' not in flags)
    return eligible,stable,flags,provisional
...
def analyze_case(rows,curve,fs,hop_ms):
    ...
    for r in rows:
        ...
        eligible,stable,flags,provisional=acoustic_assessment(r,items,attrib)
        ...
        # Decision ORDER: evaluate the interval before adjudicating the current
        # candidate. Existing penalty coefficients are never altered.
        positive_penalty=any(x['penalty'] is not None and x['penalty']>0 for x in tested)
        combined_challenge=(positive_penalty and
                            'relative_energy_low' in flags and
                            'energy_falling' in flags)
        # A challenge is NOT an automatic noise or unvoiced verdict. Do not
        # let a challenged observation enter the next consecutive-pair history.
        # Never promote a questionable low/decaying successor while the curve
        # needed to assess its incoming interval is unavailable.
        curve_missing_on_pair=(prior is not None and curve is None)
        hold_for_missing_curve=(curve_missing_on_pair and
                                'relative_energy_low' in flags and
                                'energy_falling' in flags)
        reference_admissible=(provisional and not combined_challenge and
                              not hold_for_missing_curve)
        if combined_challenge:flags.append('curve_and_low_decaying_energy_joint_challenge')
        if hold_for_missing_curve:flags.append('curve_unavailable_low_decay_reference_withheld')
        if not eligible:
        ...
        elif combined_challenge:
            diagnostic='interval_and_energy_joint_challenge_unresolved'
        elif hold_for_missing_curve:
            diagnostic='interval_not_evaluated_reference_withheld'
        elif provisional:
        ...
        r_out=dict(case=r.get('case'),sample=sample,time_s=t,
            ...
            credible_provisional_anchor=reference_admissible,
            provisional_reference_hz=eligible[0]['hz'] if reference_admissible else None,
            pred_decision_candidate_eligible=provisional,
            interval_energy_joint_challenge=combined_challenge,
            curve_missing_reference_held=hold_for_missing_curve,
            reference_withheld_reason=('joint_curve_energy_challenge' if combined_challenge else
                                       'curve_unavailable_in_low_decay_interval' if hold_for_missing_curve else
                                       'acoustic_reference_not_unique_or_not_stable' if not provisional else ''),
        ...
        previous={'sample':sample,'hz':eligible[0]['hz'] if reference_admissible else None,
                  'provisional':reference_admissible,'diagnostic':diagnostic}
    return out,counts
...
def main():
    ...
    manifest=dict(version='v12_corrected_predecision_shadow',curve_status=curve_status,cases={},
