#!/usr/bin/env python3
"""Read-only V11_04 voicing-juror audition: per-observation evidence and cross-tabs.

Run at repository root: python tests/audit_voicing_juror_v1.py --wav-root tests --out tests/voicing_juror_v1
Supply --v11-zip PATH if V11_04 ZIP is not located under --wav-root or tests/.
Requires numpy and soundfile. Does NOT alter V11 verdicts or produce MIDI.
...
def main():
    ...
    p.add_argument('--v11-zip',type=Path,default=None)
    p.add_argument('--wav-root',type=Path,default=Path('tests'))
    p.add_argument('--out',type=Path,default=Path('tests/voicing_juror_v1'))
    ...
    source=args.v11_zip
    if source is None:
        options=[args.wav_root/'shadow_adjudicator_v22_fixed_v11_04.zip',Path('tests/shadow_adjudicator_v22_fixed_v11_04.zip'),Path('shadow_adjudicator_v22_fixed_v11_04.zip')]
        source=next((x for x in options if x.is_file()),None)
        if source is None:p.error('Supply --v11-zip pointing to shadow_adjudicator_v22_fixed_v11_04.zip')
    if not source.is_file():p.error(f'Cannot find V11_04 archive: {source}')
    ...
    for case, observations in read_results(source):
        ...
        # Case-relative energy estimated only from ACTIVE V11_04 observation times;
        ...
        for ob,t,short in zip(observations,times,frames_short):
            ...
            result={'case':case,'time_s':ob['time_s'],'v11_status':outcome,
                    'v11_outcome':ob['observation_outcome'],'v11_abstention_reason':ob.get('abstention_category',''),
                    'v11_champion_hz':ob.get('tournament_champion_frequency_hz',''),
        ...
        totals[case]={'observations':len(rows),'v11':dict(outcomes),'voicing':dict(counts),
        ...
        for outcome in ('champion','unresolved'):
            for label in ('voiced_like','unvoiced_like','uncertain'):
                ...
                cross.append({'case':case,'v11_status':outcome,'voicing_class':label,'count':n,
                              'pct_of_case':round(100*n/len(rows),3) if rows else 0,
                              'pct_of_v11_status':round(100*n/outcomes[outcome],3) if outcomes[outcome] else 0})
        for outcome in ('champion','unresolved'):
            for label in ('voiced_like','unvoiced_like','uncertain'):
                matches=[r for r in rows if r['v11_status']==outcome and r['voicing_class']==label]
    ...
    for row in allrows:grand[row['voicing_class']]+=1;aggregate[(row['v11_status'],row['voicing_class'])]+=1
    summary={'method':'observational acoustic periodicity heuristic, NOT a trained human-voice detector',
             'reference_v11_zip':str(source),'reference_v11_manifest':'V11_04 observation_summary.csv',
