...
def stable_boundaries(interp, data):
    """
    Build actual boundaries between consecutive frozen STABLE_TARGET regions.

    V4.1 FIX ONLY:
    Region-index adjacency is NOT used to infer validity continuity.
    The authoritative validity wall is data.corrected_valid itself.

    A boundary is admitted only when every analysis sample from the end of the
    left stable target through the start of the right stable target is
    corrected-valid.  Intervening TRANSITION / UNRESOLVED regions are allowed.

    The boundary time remains exactly V4's midpoint of the temporal gap between
    the left stable target end and right stable target start.
    """
    regs=list(interp.regions)
    stable_pos=[i for i,r in enumerate(regs) if r.kind is RegionKind.STABLE_TARGET]
    out=[]

    corrected=np.asarray(data.corrected_valid,dtype=bool)

    for bi,(lp,rp) in enumerate(zip(stable_pos,stable_pos[1:]),1):
        left=regs[lp]; right=regs[rp]

        i=int(left.end_index)
        j=int(right.start_index)

        # Reject malformed/reversed/out-of-range spans conservatively.
        if i < 0 or j < i or j >= len(corrected):
            continue

        # Authoritative same-island test: do not bridge an actual corrected
        # validity wall.  No inference from PitchRegion adjacency is made.
        if not np.all(corrected[i:j+1]):
            continue
...
def print_timing(mm, delta_ms, midi_sec, indent="    "):
    ...
    print(
        f"{indent}timing: Δ={delta_ms:+7.1f}ms  beats={beats:+.3f}  "
...
def main():
    ...
    boundaries=stable_boundaries(interp,data)
    ...
    print("="*148)
    print("MIDI <-> FROZEN WAV PITCH-BOUNDARY CORRESPONDENCE V4.1")
    print("="*148)
