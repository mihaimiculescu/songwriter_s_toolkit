...
# =============================================================================
# V4.3 — OBSERVATIONAL ONE-SIDED RAW-CONTEXT PROBE
# =============================================================================
...
def main():
    ...
    # V4.3 observational layer.  Core V4.2 match statuses above are NOT changed.
    one_sided_probes = {
    ...
    print(
        "INSPECTION QUEUE D — V4.3 OBSERVATIONAL ONE-SIDED "
        "CORRECTED-VALID F0 PROBE"
    )
    print("=" * 148)
    print(
        "This queue does NOT create missing targets or alter V4.2 MATCHED / "
        "NO_CORRESPONDENCE status."
    )
    print(
        f"RAW_SUPPORT requires >= {RAW_WITNESS_MIN_MS:.0f}ms contiguous "
        f"corrected-valid F0 within {PITCH_LOOSE_ST:.2f}st (harmonic-aware) "
        "of the missing GT endpoint."
    )

    if not one_sided_with_raw and not one_sided_without_raw:
        print("none")

    for label, rows in (
        ("ONE_SIDED_WITH_RAW_SUPPORT", one_sided_with_raw),
        ("ONE_SIDED_WITHOUT_RAW_SUPPORT", one_sided_without_raw),
    ):
        if not rows:
            continue
        print(f"\n{label} ({len(rows)})")
        for m, p in rows:
            ...
            print(
                ...
                f"validRows={p.valid_rows} bestErr={p.best_error:.2f}st"
                f"({p.best_harmonic}) bestAt={p.best_time_s:.3f}s "
                f"exactRows={p.exact_rows} looseRows={p.loose_rows} "
                f"longestLoose={p.longest_loose_ms:.1f}ms"
            )
    ...
    print(
        "V4.3 one-sided probes read only existing corrected-valid clean_f0_hz "
