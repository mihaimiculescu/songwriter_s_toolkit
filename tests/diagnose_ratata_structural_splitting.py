...
#IN
    ...
    print(
        f"Maximal objects:      "
        f"{structural.total_objects}"
    )

    print(
        f"Observed links:       "
        f"{structural.total_links}"
    )

    print(
        f"JOIN links:           "
        f"{structural.join_links}"
    )

    print(
        f"SPLIT links:          "
        f"{structural.split_links}"
    )

    print(
        f"UNRESOLVED links:     "
        f"{structural.unresolved_links}"
    )

    print(
        f"Structural segments:  "
        f"{structural.total_segments}"
    )
    ...
    print(
        "  NO musical gesture labels are assigned"
    )
    ...
    for obj in structural.objects:
        ...
        if not obj.link_decisions:
            continue

        print(
            f"\nOBJECT "
            f"{obj.object_index:3d} "
            f"splits={obj.n_splits} "
            f"joins={obj.n_joins} "
            f"unresolved={obj.n_unresolved} "
            f"segments={len(obj.segments)}"
        )

        for decision in obj.link_decisions:
            ...
            if (
                evidence.excursion_connector
            ):
                connector_flags.append(
                    "EXCURSION"
                )
            ...
            if (
                evidence.duration_discontinuity
            ):
            ...
            if (
                evidence.center_shape_discontinuity
            ):
                boundary_flags.append(
                    "CENTER/SHAPE"
                )
            ...
            if (
                evidence.residual_discontinuity
            ):
                boundary_flags.append(
                    "RESIDUAL"
                )
            ...
            if (
                evidence.topology_discontinuity
            ):
                boundary_flags.append(
                    "TOPOLOGY"
                )
            ...
            print(
                "    "
                f"L{decision.link_index + 1} "
                f"T"
                f"{decision.left_transition_region_index}"
                f"-S"
                f"{decision.stable_region_index}"
                f"-T"
                f"{decision.right_transition_region_index} "
                f"{decision.decision.name:10s} "
                f"connector="
                f"{connector_text:<22s} "
                f"boundary="
                f"{boundary_text}"
            )
        ...
        for segment in obj.segments:
            ...
            print(
                ...
                f"T="
                f"{segment.transition_region_indices} "
                f"S="
                f"{segment.stable_region_indices} "
                f"unresolved="
                f"{segment.unresolved_link_indices}"
            )
    ...
    for obj in structural.objects:
        ...
        for decision in obj.link_decisions:
            ...
            print(
                ...
                f"{decision.decision.name:10s} "
                f"connectorFamilies="
                f"{evidence.connector_family_count} "
                f"boundaryFamilies="
                f"{evidence.boundary_family_count}"
            )
